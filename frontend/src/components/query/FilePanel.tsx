...
function DTypeBadge ({ dtype }: { dtype: string }) {
  ...
  const { label, cls } = d.startsWith('int')
    ? { label: 'int', cls: 'bg-blue-100 text-blue-800 border-blue-200' }
    : d.startsWith('float')
    ? { label: 'float', cls: 'bg-violet-100 text-violet-800 border-violet-200' }
    : d.startsWith('bool')
    ? { label: 'bool', cls: 'bg-amber-100 text-amber-800 border-amber-200' }
    : d.startsWith('datetime') || d.includes('date')
    ? { label: 'date', cls: 'bg-teal-100 text-teal-800 border-teal-200' }
    : d.startsWith('category')
    ? { label: 'cat', cls: 'bg-orange-100 text-orange-800 border-orange-200' }
    : { label: 'obj', cls: 'bg-gray-100 text-gray-600 border-gray-200' }
  return (
    <span
      className={`shrink-0 inline-block px-1.5 py-0.5 rounded border text-[10px] font-semibold font-mono tracking-wide ${cls}`}
    >
      {label}
    </span>
  )
    ...
    <div className='flex items-center gap-2'>
      ...
      <span
        className={`text-xs tabular-nums w-9 text-right font-medium ${
          pct > 0 ? 'text-amber-600' : 'text-emerald-600'
        }`}
      >
        {pct === 0 ? '✓' : `${pct}%`}
      </span>
  ...
  return (
    <div className='mt-3 rounded-lg border border-gray-100 overflow-hidden'>
      ...
      <div className='px-3 py-2 bg-gray-50 border-b border-gray-100 flex items-center gap-2 flex-wrap'>
        <span
          className='text-xs font-medium text-gray-700 truncate max-w-30'
              ...
              {columns.map(col => (
                <div
                  ...
                  className={`flex items-center justify-between gap-2 py-1 border-b border-gray-50 ${
                    preview.text_cols.includes(col) ? 'opacity-100' : ''
                  }`}
                >
                  <span
                    className={`text-xs truncate min-w-0 flex-1 ${
                      preview.text_cols.includes(col)
              ...
              <table className='w-full text-xs'>
                <thead>
                  <tr className='text-gray-400 bg-gray-50'>
                    <th className='text-left px-3 py-1.5 font-medium sticky left-0 bg-gray-50 min-w-18'>
                      column
                    </th>
                    {(['mean', 'std', 'min', 'median', 'max'] as const).map(
                      h => (
                        <th
                          key={h}
                ...
                <tbody className='divide-y divide-gray-50'>
                  {describe.map(row => (
                    ...
                    >
                      <td
                        className='px-3 py-1.5 font-medium text-gray-700 sticky left-0 bg-white max-w-18 truncate'
                        title={row.column}
                      >
                        {row.column}
                      </td>
                      {(['mean', 'std', 'min', 'median', 'max'] as const).map(
                        k => (
                          <td
                            key={k}
                            className='px-2 py-1.5 text-right text-gray-600 font-mono tabular-nums'
                          >
                            {fmt(row[k])}
                          </td>
                        )
          ...
          {missing.map(col => (
            <div key={col.column} className='px-3 py-2'>
              <div className='flex items-center gap-1.5 mb-1.5'>
                <span
                  className='text-xs font-medium text-gray-700 truncate flex-1 min-w-0'
                  title={col.column}
                >
                  {col.column}
                </span>
            ...
            <thead>
              <tr className='bg-gray-50 text-gray-400'>
                {columns.map(col => (
                  <th
                    key={col}
                    className='text-left px-3 py-1.5 font-medium whitespace-nowrap sticky top-0 bg-gray-50'
                  >
                    {col}
                  </th>
                ))}
              ...
              {sample.map((row, i) => (
                <tr key={i} className='hover:bg-gray-50 transition-colors'>
                  {columns.map(col => (
                    <td
                      key={col}
                      className='px-3 py-1.5 font-mono text-gray-600 max-w-25 truncate'
