...
def pearson_correlation(df: pd.DataFrame) -> Dict[str, float]:
    ...
    for i, j in combinations(range(len(cols)), 2):
        val = corr_matrix[i, j]
        result[f"{cols[i]}__{cols[j]}"] = round(float(val), 4) if not np.isnan(val) else 0.0
    return result
...
def categorical_correlation(df: pd.DataFrame) -> Dict[str, float]:
    ...
    if cat_df.shape[1] < 2:
        return result
    low_card_cols = [col for col in cat_df.columns if cat_df[col].nunique() <= MAX_CARDINALITY]
    ...
    for col_a, col_b in combinations(cat_df.columns, 2):
        ...
        if pair.empty:
            continue
        result[f"{col_a}__{col_b}"] = round(cramers_v(pair[col_a], pair[col_b]), 4)
    return result


def get_top_corr_pairs(pearson: Dict[str, float], cramers: Dict[str, float], top_n: int = 5) -> list:
    pairs = [
        {"col_a": k.split("__", 1)[0], "col_b": k.split("__", 1)[1], "method": "pearson", "value": v}
        for k, v in pearson.items()
    ] + [
        {"col_a": k.split("__", 1)[0], "col_b": k.split("__", 1)[1], "method": "cramers_v", "value": v}
...
def correlation_profile(df: pd.DataFrame) -> Dict[str, Any]:
    df_s = df if len(df) <= SAMPLE_SIZE else df.sample(n=SAMPLE_SIZE, random_state=RANDOM_SEED).reset_index(drop=True)
    pearson = pearson_correlation(df_s)
    cramers = categorical_correlation(df_s)
    return {
        "pearson": pearson,
        "cramers_v": cramers,
